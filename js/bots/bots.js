let referenceBots = {
	testBot : {
		grammar: {
			...
			color: ["blue", "magenta", "silver"],
			animal: ["amoeba", "emu", "unicorn"],
			mood: ["happy", "sad", "ambivalent"],
			origin: "#animal#"
		},
		states: {
			origin: {
				onEnterSay: "How do you feel?",
				exits: ["'#mood#' ->magic player.name=INPUT 'welcome, #/player/name#'", 
					"'' ->@ 'sorry, couldnt hear you'",
					"wait:10 ->idle"]
			},
			magic: {
				onEnterSay: ["Let's turn you into an animal", "what do you want to become?"],
				exits: ["'#animal#' ->idle 'I turn you into #animal.a#'", 
				"'' ->idle 'I dont know what that is' player.animal='#animal#' 'so I turn you into #color.a# #/player/animal#'"],

			},

			idle: {
				onEnterSay: ["You are idle, #/player/name#", "I give you #/objAdj.a# #/object#"],
				onEnter: ["object='#object#'", "objAdj='#objAdj#'"],
				exits: ["wait:5 ->idle 'You should do something, #/playerName# even though you have a #/object.a#'", 
					"'' ->response 'oh now you want to talk?'"
				]
			},
			response: {
				onEnterSay: "You said #/INPUT#",
				exits: ["wait:2 ->idle"]
			}
		},
		initialBlackboard: {
			player: {
				pets: ["hamster", "anaconda", "tarantula"],
				name: "Judy",
				pts: 0,
			},
			animal: "okapi",
	...
	chefBot : {
		"grammar": {
			"test": ["#personType.a# in #color.a# #personSuit#"],
			...
			"cafeName": ["#hpn#", "#name#'s", "the #surname#", "#object.capitalize# and #object.capitalize#"],
			"object": ["toaster", "teacup", "teapot", "rug","basket", "thimble", "ottoman", "cushion", "pen", "pencil", "mug","egg", "chair", "sun", "cloud", "bell", "bucket", "lemon", "glove", "moon", "star", "seed", "card", "pancake", "waffle", "car", "train", "spoon", "fork", "potato"],
			...
			"menu": "Le Menu<hr>Dessert:#food#<br><p style='font-size:10px;'><br>with<br></p> #drink#",
			"origin": "#test.capitalize#"
		...
		"states": {
			...
			"origin": {
				"onEnterSay": "You are #/player/desc#.  Let's go out to eat",
				"onEnter": "x++",
				"exits": ["'rename'->origin 'generating new name....' player.desc='#patronSingle#'", 
				"wait:5 x<10 ->chooseRestaurant player.money=(randomInt(500)+100) player.fun=0"],
				"chips": ["I want a different character", "lets go!"]
			},

		// Create 4 cafes and use them as chips
		// "chooseRestaurant": {

		// 	// Create an array of cafes, each of which has a price and a name
		// 	// Create an array of objects, arrays, 
		// 	"onEnter": `cafes=[{name:'#cafeName#', price:randomInt(0,3) + 1} for range(0,4)]`,

		// 	"chips": ["eat at home", {
		// 		"value": "cafes", 
		// 		"template": "#/value/name# ($#/value/price#)",
		// 		"exit": "#value#",
		// 	}],
		// 	"exits": ["'home' ->origin", ""]
		// },
	...
	grammar: {
		noun: ["cat", "monkey","butter", "muffin", "pants","head","socks", "demon", "fluff", "mittens", "taco", "mountain", "butt"],
		adj: ["fluffy", "fat", "puff", "nut", "dork", "glitter", "love", "unruly", "fuzz"],
	...
	states: {
		origin: {
			onEnter: "'NEW BOT TIME!!! You have a kitten!' desired_pets=randomInt(1,5)",
			exits: "->name",
		},
		name: {
			onEnter: "'What do you want to name your kitten?'",
			chips: ["#name#", "#name#", "Cupcake", "#name#"],
