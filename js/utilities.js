...
function getObjPath(obj, path) {
	console.log("Get value of obj at path", obj, path)
	...
	for (var i = 0; i < path.length; i++) {
		...
		if (typeof key === "string" && typeof obj === "object" && !Array.isArray(obj)) {
			obj = obj[key]
		} else if (typeof key === "number" && typeof obj === "object" && Array.isArray(obj)) {
			obj = obj[key]
		} else {
			console.warn(`Path error! ${path.slice(0, i).join(".")}(${typeof obj}) cannot access ${typeof key}`)
			return undefined
		}	
	}
...
function StatusTimer({key, startTime, duration, onTimeout}) {
